 ...
 */
export function parseESM2Response(response: ESM2Response, layer = 12): number[] {
  try {
    ...
    throw new Error(`Failed to parse ESM-2 response: ${error instanceof Error ? error.message : "Unknown error"}`)
  }
}
...
export function calculateEmbeddingStats(embedding: number[]) {
  ...
  const variance = embedding.reduce((sq, n) => sq + Math.pow(n - mean, 2), 0) / embedding.length
...
export function generateMockESM2Response(sequence: string): ESM2Response {
  ...
  // Normalize embedding
  const stats = calculateEmbeddingStats(embedding)
  const normalized = embedding.map((x) => (x - stats.mean) / (stats.std + 1e-8))
