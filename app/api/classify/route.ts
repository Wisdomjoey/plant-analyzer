...
import { validateSequence, cleanSequence, getSequenceStats } from "@/lib/protein-api"
import { classifyProteinWithEmbeddings } from "@/lib/classification-engine"
import { parseUniProtResponse } from "@/lib/uniprot-parser"
import { extractEmbeddingFeatures, generateMockESM2Response, parseESM2Response } from "@/lib/esm2-embeddings"
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!sequence && !uniprotId) {
      return NextResponse.json({ error: "Either sequence or uniprotId is required" }, { status: 400 })
    }

    let proteinSequence = sequence
    let sequenceId = uniprotId || "input_sequence"
    ...
    if (uniprotId && !sequence) {
      try {
        ...
        return NextResponse.json(
          { error: `Failed to fetch UniProt sequence: ${error instanceof Error ? error.message : "Unknown error"}` },
          { status: 400 },
        )
      }
    }
    ...
    if (!validateSequence(cleaned)) {
      return NextResponse.json({ error: "Invalid protein sequence format" }, { status: 400 })
    }

    if (cleaned.length < 10) {
      return NextResponse.json({ error: "Sequence must be at least 10 amino acids long" }, { status: 400 })
    }
    ...
    const classification = classifyProteinWithEmbeddings(cleaned, stats, sequenceId, embeddingFeatures)
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        embeddingStats: {
          ...
          range: [embeddingFeatures.embeddingStats.min, embeddingFeatures.embeddingStats.max],
        },
        uniprotData,
      },
    })
